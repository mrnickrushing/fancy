...
(function () {
  ...
  async function loadSettings() {
    ...
    $('set-venmo').value = s.venmo_handle || ''; $('set-applepay').checked = s.apple_pay_accepted === '1';
  ...
  $('settings-form').addEventListener('submit', async function (e) {
    e.preventDefault();
    try {
      await api('/api/admin/settings', { method: 'PUT', body: JSON.stringify({ min_notice_days: Number($('set-notice').value), deposit_percent: Number($('set-deposit').value), shipping_fee: Number($('set-shipping').value), payment_instructions: $('set-payment').value, pickup_note: $('set-pickup').value, venmo_handle: $('set-venmo').value, apple_pay_accepted: $('set-applepay').checked }) });
